...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const departmentId = String(body.departmentId || "").replace(/\D/g, "");
    ...
    if (!/^\d{3,12}$/.test(terminalNumber) || (departmentId && !/^\d{1,9}$/.test(departmentId)) || apiName.length < 5 || apiName.length > 200 || apiPassword.length < 8 || apiPassword.length > 500) {
    ...
    if (departmentId) await setPrivateConfig("cardcom_buytest_department_id", departmentId);
